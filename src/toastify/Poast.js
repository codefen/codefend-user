...
class Poast {
	...
	constructor(options) {
		...
		this.#toastElem.draggable = true; // Hacer el toast arrastrable
		this.#toastElem.addEventListener('dragstart', (event) => {
			event.dataTransfer.setData('text/plain', ''); // Necesario para algunos navegadores
			event.dataTransfer.effectAllowed = 'move';
			event.dataTransfer.setDragImage(this.#toastElem, 0, 0);
		});
		this.#toastElem.addEventListener('drag', (event) => {
			const rect = this.#toastElem.getBoundingClientRect();
			const offsetX = event.clientX - rect.left;
			const offsetY = event.clientY - rect.top;
			this.#toastElem.style.left = `${event.clientX - offsetX}px`;
			this.#toastElem.style.top = `${event.clientY - offsetY}px`;
		});
