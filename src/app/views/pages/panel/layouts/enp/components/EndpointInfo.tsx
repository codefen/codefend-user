...
export const EndpointInfo: FC = () => {
  ...
  function highlightApplicationName(titleS: string, appName: string) {
    const { before, match, after, title } = highlightToBeforeAfterMatch(titleS, appName);
  ...
  const filterByVdb = () => {
    ...
    if (filter.type === 'v') {
      sorted = [...vuln].sort((a: any, b: any) =>
        filter.order === 1 ? a.vdb_id.localeCompare(b.vdb_id) : b.vdb_id.localeCompare(a.vdb_id)
      );
  ...
  const filterByCwe = () => {
    ...
    if (filter.type === 'c') {
      ...
      sorted = [...vuln].sort(
        (a, b) => Number(extractCWEID(a.vulnerability)) - Number(extractCWEID(b.vulnerability))
      );
  ...
  const filterByTitle = () => {
    ...
    if (filter.type === 't') {
      sorted = [...vuln].sort((a: any, b: any) =>
        filter.order === 1 ? a.title.localeCompare(b.title) : b.title.localeCompare(a.title)
      );
  ...
  const filterByRisk = () => {
    ...
    if (filter.type === 'r') {
      ...
      sorted = [...vuln].sort((a: any, b: any) => a.risk_value.localeCompare(b.risk_value));
      ...
      <div className="selected-app-container">
        <div className="selected-info-app ">
          <div className="left-info">
            <div className="img-app-container">
              {selectedEndpoint?.icon_url ? (
                ...
                <span className="img-app no-img">
                  {selectedEndpoint?.application_name
                    ? selectedEndpoint?.application_name.charAt(0).toUpperCase()
                    : ''}
                </span>
              ) : (
            ...
            <div className="info-app-container">
              ...
              <p className="app-current-version">Version: {selectedEndpoint?.current_version}</p>
            </div>
          </div>
            ...
            <div className="right-info">
              <div className="right-top-info">
                <div className="vul-reported">
                  <div className="vul-reported-wrapper">
                    <p className="vul-title">Reported vulnerabilities:</p>
                    {vuln.length > 0 && vuln[0].title !== 'none' ? (
                      <div className="vul-len">
                        <p className="vul-len-text">{vuln.length}</p>
                      </div>
                    ) : vuln.length > 0 && vuln[0].title === 'none' ? (
                      <div className="vul-len">
                        ...
                        <span className="not-found-text">
                          We've found 0 indexed vulnerabilities in our scan, but this may change
                        </span>
                      </div>
                    ) : vuln.length === 0 ? (
                      <div className="waiting-container">
                        ...
                        <span className="not-found-text">
                          The vulnerabilities are pending to be scanned
                        </span>
                      </div>
                    ) : null}
                  </div>
                ...
                <div className="last-version">
                  <div className="last-version-container">
                    <p className="last-version-text">Latest version:</p>
                ...
                {compareVersions(
                  ...
                  <div className="version-installed green-version">
                    <div className="version-installed-wrapper">
                      <p className="version-installed-text text-green">Installed:</p>
                  ...
                  <div className="version-installed red-version">
                    <div className="version-installed-wrapper">
                      <p className="version-installed-text text-red">Installed:</p>
                  ...
                  <div className="version-installed empty-version">
                    <div className="version-installed-wrapper">
                      <p className="version-installed-text">
                        Installed: {selectedEndpoint?.current_version}
                      </p>
                    </div>
                  </div>
                )}
              ...
              <div className="right-botton-info">
                <div className="bottom-text-container">
                  <p className="bottom-text">Information: {selectedEndpoint?.summary}</p>
                </div>
              </div>
            </div>
          ) : (
        ...
        <div className="table-app">
          <div className="taable-app-header ">
            ...
            <p className="table-header-app-name">{selectedEndpoint?.application_name}</p>
            <p className="table-header-title">Recently reported vulnerabilities</p>
          </div>

          <div className="table-app-columns">
            <div
              className={(vulnFilter.type == 'p' ? 'special ' : '') + 'table-app-column l'}
              onClick={() => filterByDate()}>
              published
            </div>
            <div
              className={(vulnFilter.type == 'v' ? 'special ' : '') + 'table-app-column'}
              onClick={() => filterByVdb()}>
              vdb id
            </div>
            <div
              className={(vulnFilter.type == 'c' ? 'special ' : '') + 'table-app-column xl'}
              onClick={() => filterByCwe()}>
              cwe
            </div>
            <div
              className={(vulnFilter.type == 't' ? 'special ' : '') + 'table-app-column xll'}
              onClick={() => filterByTitle()}>
              title
            </div>
            <div
              className={(vulnFilter.type == 's' ? 'special ' : '') + 'table-app-column xl'}
              onClick={() => filterByScore()}>
              score
            </div>
            <div
              className={(vulnFilter.type == 'r' ? 'special' : '') + 'table-app-column'}
          ...
          <div className="table-app-rows">
            {vuln.length > 0 && vuln[0].title !== 'none'
              ? vuln.map((vulnerability: any) => (
                  <div key={vulnerability.id} className="table-row-item">
                    <div className="table-item l">{parseDate(vulnerability.timestamp_create)}</div>
                    <div className="table-item">{vulnerability.id}</div>
                    <div className="table-item xl">{extractCWEID(vulnerability.vulnerability)}</div>
                    ...
                    <div className="table-item xl vul-score">
                      <span className="score">
                        {!isNaN(parseInt(vulnerability.risk_value)) ? vulnerability.risk_value : 0}
                      </span>
                      {Array.from({ length: 5 }, (_, index) =>
                        index < (parseInt(vulnerability.risk_value) || 0) ? (
                          <div key={index} className="score-fill-balls "></div>
                        ) : (
                          <div key={index} className="score-fill-balls empty"></div>
                        )
                      )}
                    </div>
                    ...
                    <Show when={isNaN(parseInt(vulnerability.risk_value))}>
                      <div className="table-score-group">
                        ...
                        <span className="question-text">
                          The vulnerability doesn't have a category yet
                        </span>
                      </div>
                    </Show>
                  </div>
            ...
            <Show when={vuln.length > 0 && vuln[0].title == 'none'}>
              <div className="table-empty-rows">
                <div className="text">
                  There are currently no indexed vulnerabilities for this application.
                </div>
              </div>
            </Show>
          </div>
