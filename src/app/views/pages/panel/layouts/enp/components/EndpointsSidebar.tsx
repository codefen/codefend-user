...
import { ButtonLoader, PageLoader } from '@/app/views/components/loaders/Loader';
            ...
            <div className="sidebar-header-os">
              <div className="header-os-container">
                ...
                <div className="header-os-info">
                  <div className="header-os-name">{endpoints.device_os_name}</div>
                  <p className="header-os-release ">{endpoints.device_os_release}</p>
                  <p className="header-os-date">{formatDate(endpoints.creacion)}</p>
                </div>
              </div>
            </div>
            ...
            <div className="sidebar-header-results">
              <div className="header-results-founds">
                <div className="results-founds-wrapper">
                  <p className="results-found-count">{vulnerabilitiesCount}</p>
                  <p className="result-found-type">found vulnerabilities</p>
                </div>
              </div>
              <div className="header-results-founds right-no">
                <div className="results-founds-wrapper">
                  <p className="results-found-count">{endpoints.apps.length}</p>
                  <p className="result-found-type">installed applications</p>
                </div>
              </div>
            </div>
          </div>
                ...
                <div className="main-content-empty-wrapper">
                  ...
                  <div className="content-empty-message">
                    <div className="content-empty-wrapper ">
                      <div className="empty-message">the scan is currently in process</div>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {filterDuplicatesByCodeName(endpointApps)
              ...
              .map((endpoint: any) => (
                <div
                  key={endpoint.id}
                  className={
                    (endpoint.id === selectedEndpoint?.id ? 'selected ' : 'no-selected ') +
                  ...
                  <div className="card-application">
                    <div className="card-application-content">
                        ...
                        {endpoint.icon_url ? (
                          ...
                          <span className="app-img no-img-url">
                            {endpoint.application_name
                              ? endpoint.application_name.charAt(0).toUpperCase()
                              : ''}
                          </span>
                        )}
                      </div>
                      ...
                      <div className="card-app-info-container">
                        <div className="app-info-name">{endpoint.application_name}</div>
                        <p className=" app-info-org">{endpoint.organization}</p>
                        <div className="app-info-version">
                          ...
                          <Show
                            when={
                              ...
                              compareVersions(endpoint.latest_version, endpoint.current_version) ==
                                1
                            }>
                          ...
                          <Show when={endpoint.vulnerabilities && endpoint.vulnerabilities != -1}>
                            <p className="warning-icons">
                              <BugIcon />
                            </p>
                          </Show>
                    ...
                    <div className="card-selected-check ">
                      <Show when={endpoint.id === selectedEndpoint?.id}>
