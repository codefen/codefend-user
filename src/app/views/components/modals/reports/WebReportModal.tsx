...
export const WebReportModal: React.FC<WebReportModalProps> = () => {
	const { open, closeModal, resourceID } = useReportStore((state) => state);
	const [isLoading, setIsLoading] = useState(false);
	const [report, setReport] = useState<WebReport>({
		resources: [] as Webresources[],
		issues: [] as ReportIssues[],
		issueShare: {} as IssuesShare,
	});
	...
	const scopeDataTable = useMemo(
		() =>
			Number(report.resources.length) >= 1
				? report.resources.map((mainNetwork: Webresources, i: number) => ({
	...
	const vulnerabilityDataTable = useMemo(
		() =>
			Number(report.issues.length) >= 1
	...
	if (open && !isLoading) {
		const resourceDomainText = report.resources
	...
	return (
		<Show when={open && isLoading} fallback={<div></div>}>
			<ModalWrapper action={closeModal}>
				<PageLoader />
			</ModalWrapper>
		</Show>
