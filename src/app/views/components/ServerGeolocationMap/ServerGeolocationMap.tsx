...
// Country code mapping for lowercase codes
...
export const ServerGeolocationMap: FC<ServerGeolocationMapProps> = ({
  ...
  const countryData = useMemo(() => {
    ...
    const reduced = normalizeCountryData(networkData).reduce(
      (acc, item) => {
  ...
  const countriesWithServers = useMemo(() => {
    const arr = Object.entries(countryData)
      .filter(([countryName, count]) => Number(count) > 0 && COUNTRY_COORDINATES[countryName])
  ...
  // Efecto para inicializar la rotación correctamente cuando llegan los datos
  useEffect(() => {
    if (
  ...
  }, [selectedProjection, countriesWithServers.length, isInitialized]);
  ...
  const getCountryColor = (countryName: string) => {
    ...
    return lightenColor(MAP_STYLE.countryBase, lightenPercentage, theme);
  };
  ...
  const getDisplayText = () => {
    ...
    const countryCode =
      COUNTRY_CODE_MAP[currentCountry] || currentCountry.substring(0, 2).toUpperCase();
  ...
  const animateProjectionScale = (
    from: number,
    to: number,
    duration: number = MAP_STYLE.zoomOutDuration
  ) => {
  ...
  // Referencias para evitar valores obsoletos en closures
  const rotationRef = useRef(rotation);
  const animationRef = useRef<number | null>(null);
  rotationRef.current = rotation;

  // En smoothRotateTo, antes de rotar, animar suavemente de 2.0 a 1.0
  const smoothRotateTo = (
    targetCoords: [number, number],
    countryName: string,
    duration: number = MAP_STYLE.rotateDuration
  ) => {
    ...
    animateProjectionScale(
      MAP_STYLE.zoomInScale,
      MAP_STYLE.zoomOutScale,
      MAP_STYLE.zoomOutDuration
    );
    const startRotation = rotationRef.current; // Usar ref para obtener el valor actual
  ...
  useEffect(() => {
    ...
    if (selectedProjection === 'orthographicInteractive') {
      ...
      sphereGradient
        .append('stop')
        .attr('offset', '0%')
        .attr('stop-color', isDark ? MAP_STYLE.darkOceanCenter : MAP_STYLE.oceanCenter) // 🔹 COLOR: Centro del océano (blanco)
        .attr('stop-opacity', 1);

      sphereGradient
        .append('stop')
        .attr('offset', '100%')
        .attr('stop-color', isDark ? MAP_STYLE.darkOceanEdge : MAP_STYLE.oceanEdge) // 🔹 COLOR: Bordes del océano (gris muy claro)
        .attr('stop-opacity', 1);
      ...
      svg
        ...
        .attr('stroke', isDark ? MAP_STYLE.darkGlobeBorder : MAP_STYLE.globeBorder) // 🔹 COLOR: Borde del globo (gris claro)
    ...
    svg
      ...
      // ============================================
      .attr('stroke', isDark ? MAP_STYLE.darkCountryBorder : MAP_STYLE.countryBorder) // 🔹 COLOR: Bordes de países (gris claro)
    ...
    svg
      ...
      .attr(
        'stroke',
        selectedProjection === 'orthographicInteractive'
          ? MAP_STYLE.graticule3D
          : MAP_STYLE.graticule2D
      ) // 🔹 COLOR: Blanco para 3D, gris para 2D
      .attr(
        'stroke-width',
        selectedProjection === 'orthographicInteractive'
          ? MAP_STYLE.graticuleWidth3D
          : MAP_STYLE.graticuleWidth2D
      ) // 🔹 GROSOR: 0.3px para 3D, 0.5px para 2D
      .attr(
        'opacity',
        selectedProjection === 'orthographicInteractive'
          ? MAP_STYLE.graticuleOpacity
          : MAP_STYLE.graticuleOpacity
      ); // 🔹 OPACIDAD: 0.3 para ambos
    // ============================================
  }, [
    worldData,
    countryData,
  ...
  useEffect(() => {
    if (
      ...
      // Cancelar animaciones si cambiamos de proyección
      if (animationRef.current) {
        cancelAnimationFrame(animationRef.current);
        animationRef.current = null;
      }
    ...
    // Reset autoRotateIndex when changing projection or countries
    ...
    }, MAP_STYLE.autoRotateInterval);

    return () => {
      clearInterval(interval);
      // Cancelar animaciones al limpiar el efecto
      if (animationRef.current) {
        cancelAnimationFrame(animationRef.current);
        animationRef.current = null;
      }
          ...
          {currentCountry &&
            (() => {
              ...
              if (mapResources && Array.isArray(mapResources)) {
                ...
                const mr = mapResources.find(
                  (m: any) => (m.server_pais_code || '').toUpperCase() === code
                );
