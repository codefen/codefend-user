...
const isMouseNearRightPane = (x: number, rightPaneRect: DOMRect, proximityThreshold: number) => {
  return x > rightPaneRect.left - proximityThreshold && x < rightPaneRect.right;
};

const calculateOffsetX = (x: number, rightPaneRectLeft: number, proximityThreshold: number) => {
...
export const useFlashLightManager = () => {
  ...
  const handleFlashlightInRightPane = (
    ...
    if (isMouseNearRightPane(x, rightPaneRect, proximityThreshold)) {
      const offsetX = calculateOffsetX(x, rightPaneRect.left, proximityThreshold);
    ...
    if (theme === 'dark') {
      ...
      if (flashlightRef.current) {
        ...
        if (rightPaneRef.current) {
          ...
          handleFlashlightInRightPane(x, y, rightPaneGlowRadius, proximityThreshold);
        }
      }
    }
  };
