...
export const useProviderRefuseStore = create<ProviderProfileStore>((set, _get) => ({
  orderId: '',
  openRefuse: false,
  ...
  setClickRefuse: (openRefuse: boolean) =>
    set({ openRefuse, openReasonReject: false, refuseState: 0 as 0 }),
  setOpenReasonReject: (openReasonReject: boolean) =>
    set({ openRefuse: false, openReasonReject, refuseState: 1 }),
  allClose: () => set({ openRefuse: false, openReasonReject: false }),
  setIsRefusing: (isRefusing: boolean) => set({ isRefusing }),
  setRefuseState: (refuseState: 0 | 1 | 2) =>
    set((prev: ProviderProfileStore) => ({ ...prev, refuseState })),
}));
